	...
	function checkLoad(elementID) {
		setTimeout(function () {
			if (checkElement(elementID) == false) {
				...
				// Extract section name from page title here where we know the page is loaded
				var sectionName = 'Students'; // default fallback
				try {
					var pageTitleElement = document.getElementById('pageTitleHeaderTextSpan');
					bgConsole('Looking for pageTitleHeaderTextSpan element in getRoster...');

					if (pageTitleElement) {
						var fullTitle = pageTitleElement.innerText.trim();
						bgConsole('Full page title found in getRoster: ' + fullTitle);

						// Remove "Section Manager - " from the beginning and " (GRCA)" or similar from the end
						var cleanedTitle = fullTitle.replace(/^Section Manager\s*-\s*/, '').replace(/\s*\([^)]*\)$/, '').trim();
						if (cleanedTitle) {
							sectionName = cleanedTitle;
							bgConsole('Extracted section name in getRoster: ' + sectionName);
						}
					} else {
						bgConsole('pageTitleHeaderTextSpan element not found in getRoster');
					}
				} catch (error) {
					bgConsole('Error extracting section name in getRoster: ' + error.message);
				}

				scanTable(sectionName); // store the students
...
function scanTable(sectionName) {
	storage.get(null, function (result) {
		// get student rows
		let studentRows = document.querySelectorAll('#users_grid > tbody > tr:not(.th):not(.dataGridCondensed)'); // not for headers
		// get number of students for validation
		let studentCount = document.getElementById("users_grid_ctl01_lblNumRecords").innerText.match(/\d+/g)[0];
		...
		// Use the section name passed from getRoster, or fallback to default
		if (!sectionName) {
			sectionName = 'Students';
		}
		bgConsole('Using section name: ' + sectionName);

		// Get user settings to check developer mode
		const userSettings = result.userSettings || {};
		const downloadAllStudents = userSettings.downloadAllStudents || false;

		if (downloadAllStudents) {
			bgConsole('Developer mode: Download All Students is enabled');
		}
		...
		studentRows.forEach(studentRow => {
			...
			if (!teachers.includes(studentName)) {
				if (sectionStage == 'In Progress' || downloadAllStudents) {
					// create the student
					let student = {};
					...
					if (downloadAllStudents && sectionStage !== 'In Progress') {
						bgConsole('Developer mode: Added student with stage: ' + sectionStage);
					}
				} else {
