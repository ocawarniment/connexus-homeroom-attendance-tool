...
chrome.storage.local.get(null, result => {
    ...
    setTimeout(function() {
        // click Remove All
        document.querySelector('#removeAllLink_about').click();
        // wait for loading icon
        waitForElementToAppear('[src="/images/loading.gif"]', (appearedElement) => {
            console.log("Element appeared:", appearedElement);
            // wait for loading icon to go
...
function addCategory(groupName, catName) {
    ...
    let cats = document.querySelectorAll('[role="treeitem"]');

    // click groupToExpand
    cats.forEach(cat => {
        if(cat.innerText.startsWith(`${groupName} [`)) {
            groupButton = cat.parentElement.querySelector('.rtPlus');
        };
    })
