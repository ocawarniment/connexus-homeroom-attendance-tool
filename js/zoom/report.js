...
function logLiveLesson(){
    ...
    let id = window.prompt('Section ID');

    // store student list in local storage
    let attendees = [];
    let attendeeRows = document.querySelectorAll('#attendees_list_body > tr');
    attendeeRows.forEach(attendeeRow => {
        let attendeeName = attendeeRow.children[0].innerText;
        !attendees.includes(attendeeName) ? attendees.push(attendeeName) : false;
    })
    ...
    // scrape the students on the open modal
    chrome.runtime.sendMessage({type: "loglivelesson", logUrl: logUrl});
}
